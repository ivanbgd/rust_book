...
// Needs to be public because of integration tests.
pub mod constants;
...
/// Private, as it is only meant to be used by `help()`, which is public.
fn help_contents() -> String {
    let msg = format!("{HELP} {ADD} {DEPT} {DEPT_SORT} {DEPTS} {ALL} {ALL_BY_DEPT} \
...
///
/// Wraps around `help_contents()` so that function can be unit-tested.
pub fn help() {
...
pub fn add_employee(words: Vec<&str>, employees: &mut HashMap<String, Vec<String>>) {
...
pub fn print_dept(words: Vec<&str>, employees: &HashMap<String, Vec<String>>) {
...
pub fn print_dept_alternative(words: Vec<&str>, employees: &mut HashMap<String, Vec<String>>) {
    #![allow(unused)]
    // We only sort for printing in this case, and not in the original "employees" hash map.
...
pub fn print_and_sort_dept(words: Vec<&str>, employees: &mut HashMap<String, Vec<String>>) {
...
pub fn print_depts(employees: &HashMap<String, Vec<String>>) {
...
/// But, we don't need it sorted.
/// So, this just sorts the printout.
pub fn print_depts_alt(employees: &HashMap<String, Vec<String>>) {
    #![allow(unused)]
    let mut depts = employees.iter().collect::<Vec<_>>();
    depts.sort_unstable();
...
pub fn print_all(employees: &HashMap<String, Vec<String>>) {
...
/// Only sorts the printout, as HashMap cannot be sorted, at least not by using standard library.
pub fn print_all_unique_per_dept(employees: &HashMap<String, Vec<String>>) {
...
///
/// Only sorts the printout, as `HashMap` cannot be sorted, at least not by using standard library.
pub fn print_all_by_dept(employees: &HashMap<String, Vec<String>>) {
...
pub fn remove_employee(words: &Vec<&str>, employees: &mut HashMap<String, Vec<String>>) {
...
pub fn remove_dept(words: &Vec<&str>, employees: &mut HashMap<String, Vec<String>>) {
    if words.len() < 2 {
...
/// Clear the company
pub fn clear(employees: &mut HashMap<String, Vec<String>>) {
    let mut line = String::new();
...
mod tests {
    ...
    fn test_help_contents() {
        format!("{HELP} {ADD} {DEPT} {DEPT_SORT} {DEPTS} {ALL} {ALL_BY_DEPT} \
        {ALL_UNIQUE_PER_DEPT} {REMOVE} {REMOVE_DEPT} {CLEAR} {STOP}");
        let expected = "\
            help add dept deptsort depts all allbydept unique remove removedept clear stop\
        ".trim().to_string();
    ...
    fn test_add_employee() {
        let mut employees: HashMap<String, Vec<String>> = HashMap::new();
        let cmd = "add John Doe to Finance Dept";
        let words = cmd.split_whitespace().collect();
        add_employee(words, &mut employees);
        assert!(employees.contains_key("Finance Dept"));
        let dept = &employees.get("Finance Dept").unwrap();
        assert!(dept.contains(&String::from("John Doe")));
    }

    #[test]
    fn test_print_and_sort_dept() {
        let mut employees: HashMap<String, Vec<String>> = HashMap::new();
        let words = "deptsort Finance Dept".split_whitespace().collect();
        employees.entry("Finance Dept".to_string()).or_insert(Vec::new()).push("John Doe".to_string());
        employees.get_mut("Finance Dept").unwrap().push("Mark Best".to_string());
        employees.get_mut("Finance Dept").unwrap().push("Jane Doe".to_string());
        print_and_sort_dept(words, &mut employees);
        assert_eq!(employees["Finance Dept"],
                   vec!["Jane Doe".to_string(), "John Doe".to_string(), "Mark Best".to_string()]
        );
    }
    ...
    fn test_remove_employee() {
        ...
        employees.entry("Finance Dept".to_string()).or_insert(Vec::new()).push("Jane Doe".to_string());
        employees.get_mut("Finance Dept").unwrap().push("John Doe".to_string());
        employees.get_mut("Finance Dept").unwrap().push("Mark Best".to_string());
    ...
    fn test_remove_dept() {
        ...
        // Validate that it doesn't panic.
        ...
        employees.get_mut("Finance Dept").unwrap().push("John Doe".to_string());
        employees.get_mut("Finance Dept").unwrap().push("Mark Best".to_string());
        employees.entry("Engineering".to_string()).or_insert(Vec::new()).push("John Doe".to_string());
        remove_dept(&words, &mut employees);
        assert!(!employees.contains_key("Finance Dept"));
